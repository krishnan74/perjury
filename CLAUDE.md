...
**A claim submitted from `localhost` cannot settle correctly, and the failure is slow.** The enclave fetches evidence from `evidenceGatewayBaseUrl`, which is the *production* gateway. A local run records the URL in the local store, so `perjury.vercel.app/api/evidence/<id>` returns 404, the workflow cannot fetch the bundle, and the claim sits in `WitnessAssigned` — which also blocks `pendingForTribunal()` for every claim after it, since that returns the oldest. Registering it by hand unblocks it (`POST /api/evidence/<id>` with `Bearer $PERJURY_GATEWAY_TOKEN` and the gist URL), but by then the read is past `freshnessSeconds` (600) and the verdict comes back `Unverifiable` — correctly. **Rehearse live submission on the deployed site, not on `npm run dev`.**

**`/submit` renders the replay's own lanes, live.** `GET /api/live-script?runId=` calls the same `buildScript` the replay page calls, against the in-flight claim, and the page polls it every four seconds and renders `<Lanes at={beats.length}>`. Every builder underneath already returns null for a stage that has not happened, so nothing needed a live-only code path. Do not add one — a second account of the protocol is how the two pages drifted the first time.
