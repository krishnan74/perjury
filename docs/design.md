...
| | Content | Boundary |
|---|---|---|
| **Enters the enclave** | Fetched from the evidence gateway (`packages/gateway`), which is where the two agents' submissions meet — they never see each other's work. Transport and storage are both confidential: the bundle is sealed with ECIES to the tribunal's public key before it is published, so the store is a public URL holding ciphertext, and the private half is released by the Vault DON into the attested enclave alone (§3.5). The envelope is bound to its claim id as AEAD associated data, so an envelope cannot be replayed as another claim's evidence. Until Sep 10 the store was plaintext and this cell recorded that as a gap; [ADR 0010](decisions.md) closed it. Claimant's full evidence blob (raw subgraph responses, intermediate reasoning, any private data source it used), claimant's methodology description, witness's full evidence blob + methodology, both `graph-guard` provenance attestations | fetched *inside* the enclave via **Confidential HTTP** from the storage pointer; the decryption secret comes from the **Vault DON**, so node operators never see plaintext |
